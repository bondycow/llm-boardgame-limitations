...
def main():
    ...
    for index, row in df.iterrows():
        ...
        while retries < 5:  # Retry up to 5 times
            try:
                df.loc[index, 'GPT_BestMove'] = ask_gpt_for_move(row['FEN'])
    ...
    df.to_csv('gpt_cot_hard.csv', index=False)
